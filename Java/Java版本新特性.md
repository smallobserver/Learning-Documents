...
Java 8使用两个新概念扩展了接口的含义：默认方法和静态方法。[默认方法](https://lrh1993.gitbooks.io/android_interview_guide/content/java/basis/java-8.html)使得接口有点类似traits，不过要实现的目标不一样。默认方法使得开发者可以在 不破坏二进制兼容性的前提下，往现存接口中添加新的方法，即不强制那些实现了该接口的类也同时实现这个新加的方法。
...
我们可以利用jedps分析下[Spring Framework库](https://lrh1993.gitbooks.io/android_interview_guide/content/java/basis/java-8.html)，为了让结果少一点，仅仅分析一个JAR文件：**org.springframework.core-3.0.5.RELEASE.jar**。
